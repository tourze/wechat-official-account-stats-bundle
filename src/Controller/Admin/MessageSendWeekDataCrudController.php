 ...
 * 消息发送周报CRUD控制器
 */
#[AdminCrud(routePath: '/wechat-stats/message-send-week-data', routeName: 'wechat_stats_message_send_week_data')]
final class MessageSendWeekDataCrudController extends AbstractCrudController
{
    public static function getEntityFqcn(): string
    {
        return MessageSendWeekData::class;
    }

    public function configureCrud(Crud $crud): Crud
    {
        return $crud
            ->setEntityLabelInSingular('消息发送周报')
            ->setEntityLabelInPlural('消息发送周报列表')
            ->setPageTitle('index', '消息发送周报统计')
            ->setDefaultSort(['date' => 'DESC'])
            ->setSearchFields(['msgType'])
            ->setHelp('index', '微信公众号消息发送周度数据统计')
        ;
    }
