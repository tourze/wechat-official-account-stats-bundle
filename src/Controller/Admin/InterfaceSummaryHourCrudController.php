 ...
 * 接口分析时报CRUD控制器
 */
#[AdminCrud(routePath: '/wechat-stats/interface-summary-hour', routeName: 'wechat_stats_interface_summary_hour')]
final class InterfaceSummaryHourCrudController extends AbstractCrudController
{
    public static function getEntityFqcn(): string
    {
        return InterfaceSummaryHour::class;
    }

    public function configureCrud(Crud $crud): Crud
    {
        return $crud
            ->setEntityLabelInSingular('接口分析时报')
            ->setEntityLabelInPlural('接口分析时报列表')
            ->setPageTitle('index', '接口分析时报')
            ->setDefaultSort(['date' => 'DESC', 'refHour' => 'DESC'])
            ->setSearchFields(['date'])
            ->setHelp('index', '微信公众号接口分析分时数据统计')
        ;
    }
