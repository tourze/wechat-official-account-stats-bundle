...
class MessageSendMonthDataProcessor
{
    ...
    public function processResponse(Account $account, array $response): void
    {
        if (!$this->validateResponse($response)) {
            $this->logger->error('获取消息发送月数据发生错误', [
                'account' => $account,
                'response' => $response,
            ]);
    ...
    private function processDataItem(Account $account, mixed $item): void
    {
        ...
        $messageSendMonthData = $this->findOrCreateMessageSendMonthData($account, $date);
        $messageSendMonthData->setMsgUser($msgUser);
        $messageSendMonthData->setMsgCount($msgCount);
     ...
     * @return MessageSendMonthData
     */
    private function findOrCreateMessageSendMonthData(
        Account $account,
        \DateTimeImmutable $date,
    ): MessageSendMonthData {
        $messageSendMonthData = $this->messageSendMonthDataRepository->findOneBy([
            'account' => $account,
            'date' => $date,
        ]);

        if (null === $messageSendMonthData) {
