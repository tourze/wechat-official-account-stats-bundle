...
class ImageTextShareHourDataProcessor
{
    ...
    private function processDataItem(Account $account, mixed $item): void
    {
        ...
        $imageTextShareHourData = $this->findOrCreateImageTextShareHourData($account, $date);
        $imageTextShareHourData->setRefHour($refHour);
        $imageTextShareHourData->setShareScene($shareScene);
        $imageTextShareHourData->setShareCount($shareCount);
        $imageTextShareHourData->setShareUser($shareUser);

        $this->entityManager->persist($imageTextShareHourData);
    }
    ...
    {
        if (isset($item['ref_hour']) && is_int($item['ref_hour'])) {
            return $item['ref_hour'];
        }
     ...
     * @return ImageTextShareDataHour
     */
    private function findOrCreateImageTextShareHourData(
        Account $account,
        \DateTimeImmutable $date,
    ): ImageTextShareDataHour {
        $imageTextShareHourData = $this->imageTextShareDataHourRepository->findOneBy([
            'account' => $account,
            'date' => $date,
        ]);

        if (null === $imageTextShareHourData) {
            $imageTextShareHourData = new ImageTextShareDataHour();
            $imageTextShareHourData->setAccount($account);
            $imageTextShareHourData->setDate($date);
        }

        return $imageTextShareHourData;
    }
}
