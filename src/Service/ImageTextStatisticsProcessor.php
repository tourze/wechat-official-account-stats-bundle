 ...
 * 图文统计数据处理器
 */
class ImageTextStatisticsProcessor
{
    ...
    private function processItem(array $item, Account $account): void
    {
        ...
        $imageTextStatistics = $this->findOrCreateEntity($account, $date);

        $this->updateEntity($imageTextStatistics, $item);
     ...
     * 查找或创建实体
     */
    private function findOrCreateEntity(Account $account, \DateTimeImmutable $date): ImageTextStatistics
    {
        ...
        return $imageTextStatistics;
    }

    /**
     * 更新实体数据
     *
     * @param ImageTextStatistics $entity 实体
     * @param array<string, mixed> $item 数据项
     */
    private function updateEntity(ImageTextStatistics $entity, array $item): void
    {
        // 处理用户来源枚举
        $userSource = $this->extractUserSource($item);
        if (null !== $userSource) {
            $entity->setUserSource($userSource);
        }
     ...
     */
    private function extractUserSource(array $data): ?ImageTextUserSourceEnum
    {
        $userSource = isset($data['user_source']) && is_numeric($data['user_source']) ? (int) $data['user_source'] : 0;

        return ImageTextUserSourceEnum::tryFrom($userSource);
    }
