...
class MessageSendWeekDataProcessor
{
    ...
    public function processResponse(Account $account, array $response): void
    {
        if (!$this->validateResponse($response)) {
            $this->logger->error('获取消息发送周数据发生错误', [
                'account' => $account,
                'response' => $response,
            ]);
    ...
    private function processDataItem(Account $account, mixed $item): void
    {
        ...
        $messageSendWeekData = $this->findOrCreateMessageSendWeekData($account, $date);
        $messageSendWeekData->setMsgUser($msgUser);
        $messageSendWeekData->setMsgCount($msgCount);
     ...
     * @return MessageSendWeekData
     */
    private function findOrCreateMessageSendWeekData(
        Account $account,
