 ...
 * @internal
 */
#[CoversClass(SyncImageTextStatisticsHourCommand::class)]
#[RunTestsInSeparateProcesses]
final class SyncImageTextStatisticsHourCommandTest extends AbstractCommandTestCase
{
    ...
    {
        $command = self::getContainer()->get(SyncImageTextStatisticsHourCommand::class);
        $this->assertInstanceOf(SyncImageTextStatisticsHourCommand::class, $command);

        return new CommandTester($command);
    }

    private function createCommand(?OfficialAccountClient $mockClient = null): SyncImageTextStatisticsHourCommand
    {
        if (null !== $mockClient) {
            self::getContainer()->set(OfficialAccountClient::class, $mockClient);
        }

        return self::getService(SyncImageTextStatisticsHourCommand::class);
    }

    public function testConstruct(): void
    {
        $command = self::getService(SyncImageTextStatisticsHourCommand::class);
        $this->assertInstanceOf(SyncImageTextStatisticsHourCommand::class, $command);
        $this->assertSame('wechat:official-account:sync-image-text-statistics-hour', $command->getName());
        $this->assertSame('公众号-获取图文统计分时数据', $command->getDescription());
    }
    ...
    public function testExecuteWithMockedSuccessfulApiResponse(): void
    {
        ...
        $imageTextStatisticsHourRepository = self::getService(ImageTextStatisticsHourRepository::class);
        $savedData = $imageTextStatisticsHourRepository->findOneBy(['account' => $account]);
        $this->assertInstanceOf(ImageTextStatisticsHour::class, $savedData);
