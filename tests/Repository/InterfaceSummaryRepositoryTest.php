...
class InterfaceSummaryRepositoryTest extends TestCase
{
    /**
    ...
    public function testRepositoryConstruction(): void
    {
        ...
        // 测试能否正确实例化仓库
        $repository = new InterfaceSummaryRepository($registry);

        $this->assertInstanceOf(InterfaceSummaryRepository::class, $repository);
    }
    ...
    {
        // 验证实体类名被正确传递给父类构造函数
        $constructorBody = $this->getConstructorBody(InterfaceSummaryRepository::class);
        $this->assertStringContainsString('InterfaceSummary::class', $constructorBody, '构造函数应传递 InterfaceSummary::class 给父类');
    }
