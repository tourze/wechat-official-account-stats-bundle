...
class UserSummaryRepositoryTest extends TestCase
{
    /**
    ...
    public function testRepositoryConstruction(): void
    {
        ...
        // 测试能否正确实例化仓库
        $repository = new UserSummaryRepository($registry);

        $this->assertInstanceOf(UserSummaryRepository::class, $repository);
    }
    ...
    {
        // 验证实体类名被正确传递给父类构造函数
        $constructorBody = $this->getConstructorBody(UserSummaryRepository::class);
        $this->assertStringContainsString('UserSummary::class', $constructorBody, '构造函数应传递 UserSummary::class 给父类');
    }
