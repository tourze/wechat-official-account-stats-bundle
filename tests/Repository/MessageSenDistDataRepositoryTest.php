...
class MessageSenDistDataRepositoryTest extends TestCase
{
    /**
    ...
    public function testRepositoryConstruction(): void
    {
        ...
        // 测试能否正确实例化仓库
        $repository = new MessageSenDistDataRepository($registry);

        $this->assertInstanceOf(MessageSenDistDataRepository::class, $repository);
    }
    ...
    {
        // 验证实体类名被正确传递给父类构造函数
        $constructorBody = $this->getConstructorBody(MessageSenDistDataRepository::class);
        $this->assertStringContainsString('MessageSenDistData::class', $constructorBody, '构造函数应传递 MessageSenDistData::class 给父类');
    }
