...
class MessageSendWeekDataRepositoryTest extends TestCase
{
    /**
    ...
    public function testRepositoryConstruction(): void
    {
        ...
        // 测试能否正确实例化仓库
        $repository = new MessageSendWeekDataRepository($registry);

        $this->assertInstanceOf(MessageSendWeekDataRepository::class, $repository);
    }
    ...
    {
        // 验证实体类名被正确传递给父类构造函数
        $constructorBody = $this->getConstructorBody(MessageSendWeekDataRepository::class);
        $this->assertStringContainsString('MessageSendWeekData::class', $constructorBody, '构造函数应传递 MessageSendWeekData::class 给父类');
    }
