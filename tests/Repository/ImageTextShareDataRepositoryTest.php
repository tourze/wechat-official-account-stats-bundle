...
class ImageTextShareDataRepositoryTest extends TestCase
{
    /**
    ...
    public function testRepositoryConstruction(): void
    {
        ...
        // 测试能否正确实例化仓库
        $repository = new ImageTextShareDataRepository($registry);

        $this->assertInstanceOf(ImageTextShareDataRepository::class, $repository);
    }
    ...
    {
        // 验证实体类名被正确传递给父类构造函数
        $constructorBody = $this->getConstructorBody(ImageTextShareDataRepository::class);
        $this->assertStringContainsString('ImageTextShareData::class', $constructorBody, '构造函数应传递 ImageTextShareData::class 给父类');
    }
