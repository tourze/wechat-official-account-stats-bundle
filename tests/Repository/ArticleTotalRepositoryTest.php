...
class ArticleTotalRepositoryTest extends TestCase
{
    /**
    ...
    public function testRepositoryConstruction(): void
    {
        ...
        // 测试能否正确实例化仓库
        $repository = new ArticleTotalRepository($registry);

        $this->assertInstanceOf(ArticleTotalRepository::class, $repository);
    }
    ...
    {
        // 验证实体类名被正确传递给父类构造函数
        $constructorBody = $this->getConstructorBody(ArticleTotalRepository::class);
        $this->assertStringContainsString('ArticleTotal::class', $constructorBody, '构造函数应传递 ArticleTotal::class 给父类');
    }
