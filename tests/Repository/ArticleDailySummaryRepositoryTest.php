...
class ArticleDailySummaryRepositoryTest extends TestCase
{
    /**
    ...
    public function testRepositoryConstruction(): void
    {
        ...
        // 测试能否正确实例化仓库
        $repository = new ArticleDailySummaryRepository($registry);

        $this->assertInstanceOf(ArticleDailySummaryRepository::class, $repository);
    }
    ...
    {
        // 验证实体类名被正确传递给父类构造函数
        $constructorBody = $this->getConstructorBody(ArticleDailySummaryRepository::class);
        $this->assertStringContainsString('ArticleDailySummary::class', $constructorBody, '构造函数应传递 ArticleDailySummary::class 给父类');
    }
