...
class InterfaceSummaryHourRepositoryTest extends TestCase
{
    /**
    ...
    public function testRepositoryConstruction(): void
    {
        ...
        // 测试能否正确实例化仓库
        $repository = new InterfaceSummaryHourRepository($registry);

        $this->assertInstanceOf(InterfaceSummaryHourRepository::class, $repository);
    }
    ...
    {
        // 验证实体类名被正确传递给父类构造函数
        $constructorBody = $this->getConstructorBody(InterfaceSummaryHourRepository::class);
        $this->assertStringContainsString('InterfaceSummaryHour::class', $constructorBody, '构造函数应传递 InterfaceSummaryHour::class 给父类');
    }
