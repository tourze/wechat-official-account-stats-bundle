...
class AdvertisingSpaceDataRepositoryTest extends TestCase
{
    /**
    ...
    public function testRepositoryConstruction(): void
    {
        ...
        // 测试能否正确实例化仓库
        $repository = new AdvertisingSpaceDataRepository($registry);

        $this->assertInstanceOf(AdvertisingSpaceDataRepository::class, $repository);
    }
    ...
    {
        // 验证实体类名被正确传递给父类构造函数
        $constructorBody = $this->getConstructorBody(AdvertisingSpaceDataRepository::class);
        $this->assertStringContainsString('AdvertisingSpaceData::class', $constructorBody, '构造函数应传递 AdvertisingSpaceData::class 给父类');
    }
