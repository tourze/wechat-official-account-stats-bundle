...
class UserCumulateRepositoryTest extends TestCase
{
    /**
    ...
    public function testRepositoryConstruction(): void
    {
        ...
        // 测试能否正确实例化仓库
        $repository = new UserCumulateRepository($registry);

        $this->assertInstanceOf(UserCumulateRepository::class, $repository);
    }
    ...
    {
        // 验证实体类名被正确传递给父类构造函数
        $constructorBody = $this->getConstructorBody(UserCumulateRepository::class);
        $this->assertStringContainsString('UserCumulate::class', $constructorBody, '构造函数应传递 UserCumulate::class 给父类');
    }
